...
def select_action(state):
    ...
    if sample > eps_threshold:
        # print("Action from policy", policy_net(state))
        with torch.no_grad():
            return policy_net(state).max(1)[1].view(2)
    else:
        random_action = env.action_space.sample()
        # print("Random action", random_action)
        return torch.tensor(np.array(random_action), device=device, dtype=torch.long)
...
def optimize_model():
    ...
    action_batch = torch.cat(batch.action)  # .view(-1)  # Reshape to 1D tensor
    reward_batch = torch.cat(batch.reward)

    state_action_values = policy_net(
        state_batch
    )  # .gather(2, action_batch.unsqueeze(1))
...
if TRAINING:
    env = gym.make("gym_examples/JugglingEnv-v0")
else:
...
if TRAINING:
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            observation, reward, done, truncated, _ = env.step(action.cpu().numpy())
